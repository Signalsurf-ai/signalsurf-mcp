...
Every chargeable external discovery or enrichment call (`search_instagram_content`, `deepline_search_people`,
...
stable `oauth_grant_id`, `user_id`, `client_id`, `workspace_id`, `tool_name`,
...
Instagram Content Search also reserves an unbooked spend before dispatch and
books exactly three credits per requested page after an HTTP response. Transport
failures after dispatch are recorded as `ambiguous` and are never
